...
def get_color_for_group(group_idx: int) -> Color:
    ...
    return colors_list[group_idx % len(colors_list)]


def convert_bbox_coordinates(bbox: tuple, page_height: float) -> Dict:
    """
    opendataloader bbox 좌표를 PDF 좌표로 변환
    opendataloader bbox: [left, y1, right, y2] where y1 < y2
    Both use bottom-left origin - no conversion needed, just swap if needed
    """
    left, y1, right, y2 = bbox
    # y1과 y2 중 작은 값이 bottom, 큰 값이 top
    bottom = min(y1, y2)
    top = max(y1, y2)
    return {
        'x1': left,
...
def create_overlay_pdf(pdf_path: str, merged_data: Dict, original_tables: List[Dict], output_path: str, font_name: str):
    ...
    for group in merged_groups:
        ...
        for table_idx in table_indices:
            ...
            page_no = original_table.get('page', -1)
            bbox = original_table.get('bbox')
    ...
    with open(pdf_path, 'rb') as f:
        ...
        for page_num in range(num_pages):
            ...
            page_no = page_num + 1  # 1-based indexing (opendataloader uses 1-based page numbers)
...
def process_all_pdfs(input_pdf_dir: str, merged_json_dir: str, table_json_dir: str, output_dir: str):
    ...
    for json_file in json_files:
        ...
        # 원본 테이블 JSON 로드 (opendataloader가 생성한 JSON)
        source_file = merged_data.get('source_file', '')
        # source_file에서 PDF 이름 추출
        pdf_name = Path(source_file).stem
        table_json_path = os.path.join(table_json_dir, f"{pdf_name}.json")
        ...
        # 해당하는 원본 PDF 찾기
        base_name = json_file.stem.replace('_merged', '')
...
def create_legend_pdf(output_dir: str, summary_data: Dict, table_json_dir: str, font_name: str):
    ...
    for file_data in summary_data.get('files', []):
        ...
        for group in merged_groups:
            ...
            # 타이틀 표시
            table_infos_list = file_data.get('table_infos', [])
            ...
            # 연결 상세 정보
            if reasons:
                story.append(Paragraph("<b>[OK] 연결 상세:</b>", detail_style))
                for reason in reasons:
                    story.append(Paragraph(f"[OK] {reason}", detail_style))

            # 비연속 상세 정보
            if disconnections:
                story.append(Spacer(1, 0.2*cm))
                story.append(Paragraph("<b>[X] 비연속 상세:</b>", detail_style))
                for reason in disconnections:
                    story.append(Paragraph(f"[X] {reason}", detail_style))
